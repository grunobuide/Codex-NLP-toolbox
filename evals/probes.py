...
from nlp_toolbox.tools import detect_language_details
...
def langid_vs_length() -> list[tuple[str, float, float]]:
...
def main() -> int:
    print("| Input | Language-ID accuracy | English-fallback rate |")
    print("|---|---|---|")
    for label, acc, fb in langid_vs_length():
        print(f"| {label} | {acc:.4f} | {fb:.0%} |")
    return 0
