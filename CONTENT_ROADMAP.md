...
- [ ] **Stopword lists**: 100–300 entries per language, adapted from open lists (e.g. stopwords-iso, MIT) or derived from frequency counts on licensed corpora. Each list documented with source, size, and known gaps in `docs/resources.md`.
...
- [ ] **Language-specific readability formulas**, each with its academic citation: Flesch (EN), Fernández-Huerta (ES), Kandel-Moles (FR), Amstad (DE), Franchina-Vacca (IT), Martins adaptation (PT). Replaces the current English formula silently applied to every language.
